...
| `golang.org/x/sys` | `v0.12.0` | Narrow-rights Windows process-memory reads for character status. |
...
| `kernel32.dll` through `github.com/g70245/win` and `golang.org/x/sys/windows` | `OpenProcess` and `ReadProcessMemory`. | Legacy reads request access mask `0x1F0FFF`; error-returning status reads request only `PROCESS_QUERY_INFORMATION | PROCESS_VM_READ`. Required privileges and cross-integrity behavior are **To be confirmed**. |
...
| `container/battle_navigation.go` | Builds the Compact Battle navigation panel and controls its opt-in alias updater and maze runner. | Reads only while Compact Battle is active and an alias is selected; automatic movement requires an explicit Up/Down choice and Play action, and displayed text omits map names and paths. |
| `container/riding_steps_view.go` | Continuously reads remaining riding steps for every alias in a battle group. | Displays only nonzero values in Compact Battle, keeps reading in full view, and stops its updater when the group closes. |
...
| `game/character_status.go` | Reads and decodes character HP/MP and remaining riding steps from fixed supported-client addresses. | Four adjacent HP/MP XOR blocks use module offset `0xB4C308`; riding steps use `0xB4C464`; `game/constant.go` derives both addresses from the fixed `0x00400000` module base. |
...
| `internal/memory.go` | Reads absolute-address scalar/string values and error-returning byte ranges from another process. | Error-returning reads use narrow process rights, require an exact byte count, and close the process handle. The legacy path retains its zero-filled failure behavior. |
...
- **Background work:** Battle workers, production workers, the Compact Battle navigation updater and maze runner, dialog sequencing, delayed configuration notices, and audio playback use goroutines.
...
2. `newBatttleGroupContainer` creates one `game/battle.Worker` per selected `HWND`. Workers share:
   ...
   - an atomic `PartyState`
   - a `VitalsMonitor` containing HP/MP settings and one atomic movement block
...
Automatic maze traversal is a separate opt-in action inside the expanded navigation panel. The operator selects Up or Down and presses its Play control. After alias, compact-view, and battle-movement validation succeeds, Play closes all open client windows on the selected alias once before starting the runner; Stop and failed starts do not send that hotkey. The parser retains known cells from all three map sections and combines the transition section with GraphicInfo metadata for collision. GraphicInfo records are reverse-indexed by their MapID field; each map cell checks only its own ground and object MapID passability flag. GraphicInfo east/south image occupancy is not treated as collision because a tall image may visually overlap adjacent walkable cells. `0xC0xx` is normally walkable and `0xC1xx` is blocked. Live comparison of map and screen coordinates verified `0xC002` as a monster-occupied cell. Each target, Passage, and exploration calculation first treats current `0xC002` cells as snapshot-only dynamic obstacles; if that leaves no route, it immediately retries without those blocks so a monster in the only corridor does not stop traversal. Monster positions are never persisted as learned avoidance. When a selected Sandboxie game folder contains updated maps but not the base read-only graphics, only GraphicInfo resolution maps its `user\current` suffix back to the corresponding user profile; map lookup still has no recursive or host-folder fallback. Breadth-first search first chooses a reachable stair of exactly the requested type. Passage, Unknown, and opposite-direction transitions remain blocked during exploration. On initial runner start, only a Passage at or immediately next to the current position is remembered as the floor entry, preserving the behavior that steps off and re-enters a selected Up/Down stair when navigation starts on it. Automatic floor changes are identified by map code plus map name because generated maze floors may reuse the same code. After such a floor change, the nearby transition is remembered as the entry regardless of whether it is Up, Down, or Passage; this context distinguishes an entry from another identical same-direction transition without relying on shared object or transition IDs. The remembered entry is blocked from requested-target, Passage-fallback, and exploration routing. It survives stopping, restarting, and changing the Up/Down selector on the same floor identity, is replaced on the next floor change, and is cleared after leaving the maze. If the tool is first started after the character has already moved away from an entry shared by identical transitions, map data alone cannot retrospectively identify that entry and no arbitrary transition is blocked. Before either the requested stair or a usable Passage exit is revealed by the character's moving field of view, the runner follows reachable known cells toward unexplored frontiers, prefers continuing its current branch regardless of corridor width, records frontiers that remain closed after inspection, and recalculates whenever the local map file expands. Pathfinding supports all eight adjacent directions; a diagonal step requires its destination and at least one of its two neighboring cardinal cells to be walkable. Consecutive validated cells with the same direction are combined into isometric waypoints: at most four cells for cardinal movement and two cells for diagonal movement, including beside walls and passable overlays, so the client can perform its own local obstacle adjustment without receiving an off-client click. A known live starting cell may leave the current position even if stale collision data marks that starting cell blocked. The runner polls navigation progress every 50 milliseconds, monitors live coordinates without stopping at every cell, and sends movement through a navigation-only click path with a 50-millisecond cursor delay and 30-millisecond post-click delay; shared item, skill, battle, and production clicks retain their existing timing. It replans after roughly 300 milliseconds without movement and temporarily excludes only the exact failed next cell. If transient blocks or completed frontiers leave no route, it clears that per-run state once and recalculates automatically, matching the recovery previously obtained by Stop and Play. If navigation starts while the character is already standing on the selected stair, it moves to an adjacent safe non-transition cell and then re-enters the stair; a stair reached by the runner is not stepped off again while its transition completes. It pauses unless every window in the group is in the normal scene, so a party leader cannot move while another party member remains in battle. It clears per-run exact blocks, failure counts, completed frontiers, and heading on restart. It stops after the selected window leaves the maze map tree and is cancelled by its Stop control, `Navigation Off`, alias changes, leaving compact view, group deletion, refresh, or application cleanup. Existing configured battle movement must be `None` to prevent two movement systems from sending competing clicks.
At a direction change beside a known blocked cell, the segment builder may include exactly one validated path cell beyond the corner and reports that outgoing direction as the new heading. This avoids making a concave wall corner itself a click destination while leaving open-area turns, unknown fog boundaries, backward turns, and the normal four/two-cell limits unchanged.

Floor-identity changes invalidate cached map contents even when a generated maze reuses the same `.dat` path, size, and modification timestamp. A changed maze floor is accepted only after a stable map-code/name read and a transition appears at or adjacent to the spawn position; until then the runner reports Loading and retries without issuing movement.

Continuous traversal keeps the remembered floor entry blocked only while the selector continues in the direction active when that floor was entered. Reversing Up/Down keeps the entry coordinates but stops applying their block, allowing a new Runner to route back to a known entry even after the character has moved away; changing back restores the original block. A new Play beginning directly on a remembered entry whose type matches the explicit selection also overrides only the current cell, allowing the runner to step off and re-enter it without weakening automatic entry avoidance after later floor changes.

Requested Up/Down targets retain first priority and Passage retains its existing fallback priority. If neither is reachable and at least two stairs of the opposite type are visible, the runner treats them as exit candidates, locks the nearest untried candidate for the duration of that attempt, and records it only after the transition reaches another maze floor. On that floor, a selected-direction transition under the spawn is temporarily allowed so the existing step-off/re-entry behavior returns through it; returning to the duplicate-stair floor then excludes the tried candidate and selects another. Candidate attempts are stored separately from remembered floor entries and are cleared when the Up/Down selector changes; entry coordinates and their arrival direction remain available for direction-scoped blocking.

Navigation Play requires configured alert music so verification monitoring cannot silently fail. During the Runner lifetime, an injected callback independently reuses `game.IsVerificationTriggered` about every 500 milliseconds before reading a movement snapshot or issuing a click; it does not share Battle or Production worker tickers and does not alter their log behavior. A match reports `Verification required`, invokes the injected beeper callback once, and ends the Runner. Pressing Play again stops that alert before restarting, while the application-wide Ctrl+0 shortcut remains available to mute it directly.
...
Magic Baby uses random encounters on normal maps and turn-based party battles. Party windows can leave the battle scene at different times. When the group-level `Party` control is enabled, `sharedWaitGroup` deliberately tracks party members still executing battle actions: a character already back in the normal scene waits before moving so the leader cannot trigger another encounter while teammates remain in battle. The control is off by default because windows in one application group do not necessarily form one in-game party.
...
flowchart TD
    ...
    Scene -->|NORMAL_SCENE| StopReason{lure or inventory stop?}
    StopReason -->|No| Vitals{Any HP/MP monitor enabled?}
    Vitals -->|Yes| ScanVitals[Read every group window's character and active-pet status]
    Vitals -->|No| MoveGuard{Group vitals blocked?}
    ScanVitals --> VitalsResult{Low or unreadable value?}
    VitalsResult -->|Low| VitalsAlert[Block group movement and play alert once]
    VitalsResult -->|Read failure| Pause
    VitalsResult -->|Healthy| MoveGuard
    VitalsAlert --> Pause
    MoveGuard -->|No| PartyWait{Party enabled?}
    PartyWait -->|Yes| WaitParty[Wait for group battle actions]
    PartyWait -->|No| Move[MovementState.Move]
    WaitParty --> Move
    ...
    FlawlessAlert --> Loop
    Flawless -->|No| Character[Character action state machine]
...
`ActionState` transforms configured action records into mouse/key operations. It detects stages and results by pixels, changes action IDs according to `StartOver`, `Continue`, `Repeat`, or `Jump`, and resets IDs after battle. Targeted character and pet actions still inspect fixed player/enemy coordinates. HP and MP checks are not battle actions; they are independent runtime-only group monitors. Movement reads current map coordinates from process memory and clicks a point around the 640×480 center.

The optional `HP`, `Pet HP`, `MP`, and `Pet MP` controls store separate group-level ratios. HP options range from 10% through 100%; MP options range from 5% through 55% in 5% increments. Before each movement attempt, any worker whose movement mode is not `None` scans every game window in its external application group. One 64-byte read obtains all four character HP/MP blocks whenever either character monitor is enabled. Character HP alone also reads remaining riding steps: while riding with at least 150 remaining steps, its configured ratio is halved; at zero through 149 steps it remains unchanged. MP ratios are never adjusted for riding. Pet monitors scan all five local slots in each process, compare only state-2 pets, and use one four-block status read per active slot. A below-ratio result atomically blocks group movement, starts the shared alert once, and pauses each moving worker when it next reaches the normal scene. A required memory-read failure also blocks and pauses movement but does not start audio. Healthy reads never clear the block; the next group Play resets it. Workers revalidate the shared block immediately before `MovementState.Move`, while movement itself remains independent and unchanged.

HP, Pet HP, MP, Pet MP, and Flawless Pet cannot be enabled until alert music is configured. Party requires no alert configuration. Other monitoring controls retain their existing reminder or validation behavior.
...
Self-target detection also captures one current client frame, then checks player-existence pivots and self-name colors in the RGBA buffer. The caller-provided preference still determines whether character positions or pet positions are searched first because the supported client pixels do not reliably distinguish those roles.
...
| `VitalsMonitor` | `game/battle` | Holds four runtime-only group HP/MP ratio settings and the atomic movement block, and scans every group window before movement. | `game.ReadCharacterStatus`, `game.ReadRidingSteps`, `game.ReadPetStatus` | `battle.Worker`, battle-group Monitoring UI | Each moving worker scans independently; only the block state is shared. |
...
- The Flawless Pet checker is a per-worker atomic runtime flag. Party, all four HP/MP settings, and the shared vitals movement block are runtime-only group state. Like the other enabled checkers, none are part of saved `.ac` action configuration.
...
- Worker checker/stop flags, `PartyState`, the group inventory status, and the vitals movement block use atomics. Movement, enemy-order, action, game-directory, and production-name access use synchronized APIs. Battle execution uses a deep action snapshot rather than UI-owned slices.
...
- While Party is enabled, `sharedWaitGroup` represents group windows still in Magic Baby's turn-based battle scene. A window back in the normal scene waits before moving, preventing the leader from starting another random encounter while teammates are still leaving battle. `Done` is deferred around each grouped action so the count is released on every action return path.
- `sharedInventoryStatus` is a shared `atomic.Bool`.
- `VitalsMonitor.blocked` is a shared `atomic.Bool`. Workers scan independently and revalidate it immediately before movement; this guard deliberately avoids synchronizing their movement schedules.
...
- `internal.readMemory` closes each process handle after its read attempt, including read-failure paths. `ReadMemoryAtAddress` closes its narrow-rights process handle and reports read, short-read, and close failures.
...
- Legacy process-memory read failures are not surfaced to users. Character and active-pet status readers return exact-read errors; the vitals monitor logs the failure, blocks movement, and does not play audio. Riding-step display failures appear as `ERR` beside the affected alias.
...
The repository contains focused unit tests for enum option conversion, process-handle ownership, exact fixed-address reads, four-block character HP/MP XOR decoding and ratio comparison, group vitals scanning and blocking, riding-step decoding and Compact Battle presentation, log/filesystem behavior, log-directory validation, user-facing setup messages and action-ID validation, action-configuration I/O, and the synchronized audio lifecycle. These tests use pure values, fake native operations, temporary filesystem fixtures, and fake audio sessions; they do not require a live game window, process memory, user log directory, or audio device.
...
- Pet target, inventory, item, and production pixel detection; live compatible-client validation of character and pet HP/MP remains manual even though decoding, ratio logic, and monitor coordination are unit tested.
