...
| --- | --- | --- | --- | --- |
| Process handles are not closed | `internal/memory.go:readMemory` | Every map-name or position read opens a process handle without `CloseHandle`. Long-running movement/checker workflows can exhaust handles. | Memory reads occur repeatedly on tickers and movement decisions. | Measure handle growth, add error-aware ownership, and guarantee `CloseHandle` after reads. |
