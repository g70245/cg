...
- Replaced the built-in Activities log phrase for wild level-one encounters with an independent runtime-only `Level 1` monitor. It reads `actor + 0x1D0` across left-side battle slots `10..19`, alerts and pauses battle actions on a match, runs before the independent Flawless Pet check, and leaves read failures as logged non-blocking errors; Monitoring now places `Activities` last.
